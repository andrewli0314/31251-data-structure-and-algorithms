...
std::vector<std::pair<vertex, vertex>> random_tree(
  ...
  if (!vertices.empty()) {
    ...
    for (int i = 1; i < vertices.size(); ++i)
      unconnected.push_back(vertices[i]);
...
void make_strongly_connected(directed_graph<T>& d) {
  ...
  for (auto i = 0; i < vertices.size(); ++i) {
    d.add_edge(vertices[i % vertices.size()],
 ...
 public:
  ...
  void testIsDagDisconnectedDag() {
    ...
    while (start < verts.size()) {
      ...
      start = end;
      if (verts.size() - start > 0)
        end = 5 + start + rand() % (verts.size() - start);
    }
  ...
  void testIsDagBiggerNonDag() {
    ...
    for (auto i = 1; i < verts.size(); ++i)
      if (rand() % 2) d.add_edge(ordered_verts[0], ordered_verts[i]);
    for (auto i = 2; i < verts.size(); ++i)
      if (rand() % 2) d.add_edge(ordered_verts[i], ordered_verts[1]);
  ...
  void testIsHamiltonianDagNoPath() {
    ...
    for (auto i = 0; i < ordered_verts.size(); ++i) {
      if (rand() % 2)
        left.add_vertex(ordered_verts[i]);
      else
        right.add_vertex(ordered_verts[i]);
    }
    ...
    d.add_vertex(root);
    d.add_edge(root, ordered_verts[0]);
    d.add_edge(root, ordered_verts[1]);
